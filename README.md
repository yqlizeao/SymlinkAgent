...
<p align="center">
<img src="docs/screenshot-main.png" alt="SymlinkAgent 图形界面" width="760"/>
</p>
...
SymlinkAgent 解决一个具体问题:你在多个目录里都要用到相同的配置文件(`.mcp.json`、`.claude/`、自定义脚本等),但不想在每个目录里都维护一份拷贝。

做法很简单:把所有需要共享的文件放在一个**源目录**里,用文件资源管理器直接管理;SymlinkAgent 负责把它们以**符号链接**的方式挂到各个目标目录。目标目录始终保持干净——只有链接、没有实体文件。
...
**约定**:源目录顶层的每个文件/目录,自动成为一个链接条目——无需任何清单或配置文件。
...
从 [Releases](../../releases) 下载最新版本,解压到任意目录即可使用(免安装,免 .NET 运行时)。
...
```bash
# 1. 把某个文件夹登记为源目录
SymlinkAgent init D:\my-config-source
...
CLI 命令与 GUI 视图模型都是 `TargetEngine` 的薄壳,业务逻辑只有一份。

## 详细规格

链接契约、状态格式、命令语义、冲突与删除安全规则 → [docs/spec.md](docs/spec.md)
